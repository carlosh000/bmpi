...
Modo recomendado (sincroniza con base, valida y sube):

```bash
./scripts/sync_and_push_branch.sh origin main safe
```

Modo rápido (sincroniza/rebasea y sube, sin pruebas):

```bash
./scripts/sync_and_push_branch.sh origin main sync-no-tests
```

Modo urgente (solo subir avance actual, sin rebase):
