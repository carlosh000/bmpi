...
REMOTE="${1:-origin}"
BASE_BRANCH="${2:-main}"
MODE="${3:-safe}" # safe | sync-no-tests | push-only
...
if [[ "${MODE}" == "push-only" ]]; then
  echo "[push-only] Skipping fetch/rebase/tests."
  echo "Pushing ${CURRENT_BRANCH} -> ${REMOTE}/${CURRENT_BRANCH} with --force-with-lease..."
...
echo "[2/5] Rebasing ${CURRENT_BRANCH} onto ${REMOTE}/${BASE_BRANCH}..."
set +e
...
if [[ ${REB_EXIT} -ne 0 ]]; then
  echo
  echo "Rebase stopped due to conflicts. Resolve and continue:"
  echo "  git status"
  echo "  git add <resolved-files>"
  echo "  git rebase --continue"
  echo "If you prefer to upload immediately without rebasing:"
  echo "  ./scripts/sync_and_push_branch.sh ${REMOTE} ${BASE_BRANCH} push-only"
  exit ${REB_EXIT}
fi
...
if [[ "${MODE}" == "safe" ]]; then
  ...
  echo "[3/5] sync-no-tests mode: skipping build/tests."
fi
